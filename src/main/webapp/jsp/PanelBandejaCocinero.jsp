            ...
            <section class="dashboard-cocinero p-4">
                <div class="p-2 m-b-2">
                    <a class="rojo-texto texto-none texto-hover"
                        href="${pageContext.request.contextPath}/jsp/PanelCocinero.jsp">
                ...
                <div class="pedidos-grid">
                    <c:forEach items="${pedido}" var="p">
                        <c:if test="${p.estadoPedido != 'LISTO'}">
                            <c:set var="estiloTarjeta"
                                value="${p.estadoPedido == 'PENDIENTE' ? 'blanco-bg' : 'azul-bg-light'}" />
                            <c:set var="estiloEtiqueta"
                                value="${p.estadoPedido == 'PENDIENTE' ? 'amarillo-bg' : 'azul-bg'}" />
                            ...
                            <div class="tarjeta-pedido-activo ${estiloTarjeta} borde-redondeado p-2 box-shadow m-b-2">
                                <div class="d-flex space-between m-b-1">
                                    ...
                                    <span
                                        class="etiqueta-estado ${estiloEtiqueta} gris-texto texto-negrita h7 borde-redondeado p-t-7 p-b-7 p-i-1">
                                        ${p.estadoPedido}
                                    </span>
                                </div>
                                    ...
                                    <form action="${pageContext.request.contextPath}/PedidosController" method="POST">
                                        ...
                                        <c:choose>
                                            <c:when test="${p.estadoPedido == 'PENDIENTE'}">
                                                <button type="submit"
                                                    class="boton borde-none rojo-bg blanco-texto p-1 pointer">
                                                    Iniciar Preparación
                                                </button>
                                            </c:when>
                                            <c:otherwise>
                                                <button type="submit"
                                                    class="boton borde-none verde-bg blanco-texto p-1 pointer">
                                                    Terminar Pedido
                                                </button>
                                            </c:otherwise>
                                        </c:choose>
